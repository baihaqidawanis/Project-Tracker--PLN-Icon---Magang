...
export default function TabNavigation({ activeTab, setActiveTab, collapsed }: TabNavigationProps) {
  return (
    <div
      className={`fixed left-0 top-[60px] bottom-0 bg-gray-900 dark:bg-gray-950 border-r border-gray-700 dark:border-gray-800 transition-all duration-300 z-40 ${collapsed ? 'w-16' : 'w-64'
        }`}
    >
        ...
        {tabs.map(tab => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-3 px-4 py-3 transition-colors relative group ${activeTab === tab.id
